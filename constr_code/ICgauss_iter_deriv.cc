...
void pbc(long n, std::complex<MyFloat> *delta_in, std::complex<MyFloat> *delta_out, long r1, long r2, long r3){
...
//optional: shift position to center e.g. on specific halo       
	...
	free(rnd_t);
       ...
       complex<MyFloat> *mean=(complex<MyFloat>*)calloc(npartTotal,sizeof(complex<MyFloat>)); //==0 for our case
...
//        ofstream ofs3("ang_arr3.txt");
	    ...
	    //part_arr[i]=i;
	    ang_arr1[i]=cen_deriv4_alpha(&grid, part_arr[i], alpha_mu1, dx, 0, pot0);
	    ang_arr2[i]=cen_deriv4_alpha(&grid, part_arr[i], alpha_mu2, dx, 1, pot0);
	    ang_arr3[i]=cen_deriv4_alpha(&grid, part_arr[i], alpha_mu3, dx, 2, pot0);
...
//end of L-alpha calculation  
   ...
   /*
    cout<< "values for alpha_mu*phi (after constraint, real & k-space): " << nsLx1 <<  " " << nsLxk1 << "  " << nsLx2 <<  " " << nsLxk2 <<  " " << nsLx3 <<  " " << nsLxk3<< endl;      
     ...
     complex<MyFloat> psu8 (0.,0.);
     complex<MyFloat> prsu6 (0.,0.);
     complex<MyFloat> prsu8 (0.,0.);
   ...
   */
	...
	for(iy=0;iy<res;iy++){
	  ...
	  pbc(n, psift1_t, psift1, -s1, -s2, -s3);
	  pbc(n, psift2_t, psift2, -s1, -s2, -s3);
	  pbc(n, psift3_t, psift3, -s1, -s2, -s3);
