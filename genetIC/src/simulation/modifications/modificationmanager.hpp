  ...
  private:
    ...
    std::shared_ptr<LinearModification<DataType, T>> getLinearModificationFromName(std::string name_) {
      if ((strcasecmp(name_.c_str(), "overdensity") == 0)) {
        ...
        return make_shared<AngularMomentumModification<DataType, T>>(multiLevelContext, cosmology, 0);
      } else if ((strcasecmp(name_.c_str(), "Ly") == 0)) {
        return make_shared<AngularMomentumModification<DataType, T>>(multiLevelContext, cosmology, 1);
      } else if ((strcasecmp(name_.c_str(), "Lz") == 0)) {
        return make_shared<AngularMomentumModification<DataType, T>>(multiLevelContext, cosmology, 2);
      } else {
