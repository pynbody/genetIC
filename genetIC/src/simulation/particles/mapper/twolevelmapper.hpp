    ...
    public:
      ...
      */
      ...
      /*! \brief Returns an iterator pointing at the beginning of the list of particles with the specified type
        ...
        Note that this is somewhat fragile. In particular, if a gadget particle type appears twice in the
        hierarchy constructed from TwoLevelMappers, in such a way that the particle type changes from A to B and
        then back to A again when iterating through the mapper as a whole... the results are undefined. If a user
        asks for the same particle type in different parts of their output, an error is generated by the front-end.
        However this is not enforced anywhere else (including here -- as I say, the behaviour will just be undefined).
        One day the whole system for treating gadget particle types would benefit from a rethink.
      */
...
#pragma omp critical
      ...
      */
      ...
      MapPtrType insertIntermediateResolutionPadding(size_t ratio, size_t padCells) override {
        ...
        if (newCellLengthRatio > ratio) {
          ...
          */
          ...
          mapperForPaddingRegion->setGadgetParticleType(mapperLR->getGadgetParticleTypeForFinestGrid());
      ...
      unsigned int getGadgetParticleTypeForFinestGrid() override {
        return pLevel2->getGadgetParticleTypeForFinestGrid();
      }
    ...
    protected:
      ...
      void debugInfoForIterator(std::ostream &s, int n, const iterator *pIterator) const override {
        ...
        if(pIterator->subIterators.size()==0) {
          s << "      ... subIterators not available" << endl;
          return;
        }
        ...
        if (i >= firstLevel2Particle) {
          ...
          if(pIterator->subIterators[1]==nullptr)
            s << "                     ---> nullptr" << endl;
          else
