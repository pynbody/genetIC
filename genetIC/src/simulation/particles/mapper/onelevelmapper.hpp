    ...
    public:
      ...
      */
      std::pair<MapPtrType, MapPtrType> splitMass(T massRatio, const std::vector<GridPtrType> &toGrids) override {
        auto s1 = std::make_shared<OneLevelParticleMapper<GridDataType>>(
          this->pGrid->makeScaledMassVersion(massRatio));
        auto s2 = std::make_shared<OneLevelParticleMapper<GridDataType>>(
        ...
        if(!pGrid->isProxyForAnyOf(toGrids))
          s1 = std::shared_ptr<OneLevelParticleMapper<GridDataType>>(nullptr);
