...
\begin{itemize}
    ...
    inverse fourier transforming the $\tilde{\vec{\delta}}^{\low}$. It can be
    sampled on a coarser grid of points than the original $\vec{\delta}$
...
I will try hard to stick to these notations.

\boxme{{\bf The key point:} We cannot store $\vec{\delta}$, which is the full resolution
overdensity vector across the full box at high resolution. However, we can store $\vec{\delta}^{\low}$ which is the
low-resolution vector across the full box; we can also store $\vec{\delta}^{\high,W}$
which is the high-k part of the vector in the subbox; finally, we can also store
...
ultimately to express everything in terms of ``low", ``high,W" or ``W" quantities.
The final output consists of the ``low" (unzoomed particles) and the ``W" (zoom particles) vectors.}
