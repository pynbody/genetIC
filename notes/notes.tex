...
How would we get a realization of $\delta^{\low}$ and $\delta^{\high}$? There
are two clear possibilities. First, if it's possible to get a realization
of $\vec{\delta}$, we simply need to define
\begin{equation}
  \delta^{\low} = \mathsf{F}^{\low} \delta \hspace{1cm} \delta^{\high} = (1 - \mathsf{F}^{\low}) \delta\label{eq:decompose-delta}
\end{equation}
...
\begin{equation}
\vec{\delta}^R = \mathsf{F}^{\low} \mathsf{C}^{1/2} \vec{q}^{\low} \hspace{1cm} \delta^{\high} = \mathsf{F}^{\high} \mathsf{C}^{1/2} \vec{q}^{\high}
\end{equation}
where $\vec{q}$ are Gaussian white
noise. This is, for most (but not all) purposes, what we will adopt in practice -- since
we can't actually draw from the full summed field $\delta$ (that's the whole point in the first place).
But what should $\mathsf{F}^{\high}$ be? To check, we need to
get $\mathsf{C}^{\low}$ and $\mathsf{C}^{\high}$:
\begin{equation}
...
\begin{equation}
(\mathsf{F}^{\low})^2+ (\mathsf{F}^{\high})^2 = \mathsf{I}\textrm{,}
\end{equation}
...
a Fermi function for $\mathsf{F}^{\low}$,
\begin{equation}
  \mathsf{F}^{\low}=\frac{1}{1+\exp\left(\frac{k-k_{\mathrm{cut}}}{T}\right)}
\end{equation}
...
With $\mathsf{F}^{\low}$ fixed, there isn't a unique $\mathsf{F}^{\high}$ because
it depends on which of the above techniques we're using. For the toy code,
{\tt filter\_low} and {\tt filter\_high} implement $\mathsf{F}^{\low}$ and $1-\mathsf{F}^{\low}$
whereas the {\tt C\_filter\_low} and {\tt C\_filter\_high} implement $(\mathsf{F}^{\low})^2$ and $1-(\mathsf{F}^{\low})^2$.
These functions are also named the same in the production code, and can be found in the
{\tt IC} class.

\subsection{Introducing the window}

Now we also have to window the high-resolution part, i.e.
\begin{align}
\vec{\delta}^{\high} & = \mathsf{W} \mathsf{F}^{\high} \mathsf{C}^{1/2}
\vec{q}^{\high}\\
...
\mathsf{F}^{\high} \mathsf{C}^{1/2}$ .  Then within the window, we have the
...
$\vec{\delta}^{\high}$, subject to knowing only $\vec{q}^{\high}_W$, which
gives us
\begin{equation}
\vec{\delta}^{\high} = \mathsf{WC_{\high}^{1/2}W}^{\top} (\mathsf{WW}^{\top})^{-1} \vec{q}^{II}_W \textrm{.}
\end{equation}
...
Then we are starting with a vector $\vec{\alpha}$ defined inside the window.
...
example could be more problematic.}
